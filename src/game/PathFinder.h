...
class PathInfo
{
    public:
        ...
        G3D::Vector3 const& getStartPosition() const { return m_startPosition; }
        G3D::Vector3 const& getNextPosition() const { return m_nextPosition; }
    ...
    private:
        ...
        Movement::PointsArray       m_pathPoints;       // our actual (x,y,z) path to the target
        ...
        G3D::Vector3        m_startPosition;    // {x, y, z} of current location
        G3D::Vector3        m_nextPosition;     // {x, y, z} of next location on the path
        G3D::Vector3        m_endPosition;      // {x, y, z} of the destination
        G3D::Vector3        m_actualEndPosition;  // {x, y, z} of the closest possible point to given destination
        ...
        inline void setNextPosition(G3D::Vector3 point) { m_nextPosition = point; }
        inline void setStartPosition(G3D::Vector3 point) { m_startPosition = point; }
        inline void setEndPosition(G3D::Vector3 point) { m_actualEndPosition = point; m_endPosition = point; }
        inline void setActualEndPosition(G3D::Vector3 point) { m_actualEndPosition = point; }
...
inline bool inRange(const G3D::Vector3 p1, const G3D::Vector3 p2,
