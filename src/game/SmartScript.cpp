...
bool SmartScript::ProcessAction(SmartScriptHolder &e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellEntry* spell, GameObject* gob)
{
    //calc random
    ...
    switch (e.GetActionType())
    {
        ...
        {
            if (!unit || !unit->ToPlayer()) return true;//return if no player
        ...
        {
            if (!unit || !unit->ToPlayer()) return true;//return if no player
            if (const Quest* q = objmgr.GetQuestTemplate(e.action.quest.quest))
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); itr++) {
                    if (IsUnit((*itr)))
                    {
                        ...
                        if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !((Unit*)(*itr))->HasAura(e.action.cast.spell))
                            if(!me->CastSpell(((Unit*)(*itr)), e.action.cast.spell,(e.action.cast.flags & SMARTCAST_TRIGGERED) ? true : false))
                                return false;
                    }
                }
                break;
        ...
        /*case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:    //FIXME
            {
                if (me)
        ...
        case SMART_ACTION_CALL_FOR_HELP:
            {
                if (me)
                    me->CallAssistance(/*(float)e.action.callHelp.range*/); //FIXME
                break;
            }
        ...
        /*case SMART_ACTION_SET_INVINCIBILITY_HP_LEVEL: //FIXME
            {
        ...
        case SMART_TARGET_CREATURE_RANGE:
            {
                ...
                for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); itr++)
                {
                    ...
                    if (me && me == (*itr))
                        continue;
                    if (((e.target.unitRange.creature && (*itr)->ToCreature()->GetEntry() == e.target.unitRange.creature) || !e.target.unitRange.creature) && GetBaseObject()->IsWithinDistInMap((*itr), (float)e.target.unitRange.minDist, (float)e.target.unitRange.maxDist))
                        l->push_back((*itr));
                }
                break;
            }
        ...
        case SMART_TARGET_GAMEOBJECT_RANGE:
            {
                ...
                for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); itr++)
                {
                    ...
                    if (go && go == (*itr))
                        continue;
                    if (((e.target.goRange.entry && IsGameObject((*itr)) && (*itr)->ToGameObject()->GetEntry() == e.target.goRange.entry) || !e.target.goRange.entry) && GetBaseObject()->IsWithinDistInMap((*itr), (float)e.target.goRange.minDist, (float)e.target.goRange.maxDist))
                        l->push_back((*itr));
                }
                break;
            }
...
void SmartScript::ProcessEvent(SmartScriptHolder &e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellEntry* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_TARGET_HEALTH_PCT:
            {
                ...
                if(ProcessAction(e, me->getVictim()))
                    RecalcTimer(e, e.event.minMaxRepeat.repeatMin, e.event.minMaxRepeat.repeatMax);
                break;
            }
        ...
        case SMART_EVENT_FRIENDLY_IS_CC:
        {
            ...
            if (pList.empty())
                return;
            if(ProcessAction(e, *(pList.begin())))
        ...
        case SMART_EVENT_SPELLHIT:
        {
            ...
            if (!e.event.spellHit.spell || spell->Id == e.event.spellHit.spell)
                ...
                {
                    if(ProcessAction(e, unit, 0, 0, bvar, spell))
        ...
        case SMART_EVENT_DATA_SET:
        {
            if (e.event.dataSet.id != var0 || e.event.dataSet.value != var1)
                return;
            if(ProcessAction(e, unit, var0, var1))
