...
{
    GetNearPoint(obj, x, y, z, 0.0f, distance2dMin+(distance2dMax-distance2dMin)*GetMap()->rand_norm(), GetAngle(obj));
}
...
void Unit::MovePositionToFirstCollision(uint32 mapId, Position &pos, float dist, float angle)
{
...
void Unit::GetFirstCollisionPosition(uint32 mapId, Position &pos, float dist, float angle)
{
    GetPosition(pos.m_positionX, pos.m_positionY, pos.m_positionZ);
    MovePositionToFirstCollision(mapId, pos, dist, angle);
}

void Unit::UpdateAllowedPositionZ(float x, float y, float &z) const
{
    switch (GetTypeId())
    {
        case TYPEID_UNIT:
        {
            ...
            if (!ToCreature()->canFly())
            {
                ...
                float ground_z = z;
                float max_z = canSwim
                    ? GetBaseMap()->GetWaterOrGroundLevel(x, y, z, &ground_z, !HasAuraType(SPELL_AURA_WATER_WALK))
        ...
        case TYPEID_PLAYER:
        {
            ...
            {
                float ground_z = z;
                float max_z = GetBaseMap()->GetWaterOrGroundLevel(x, y, z, &ground_z, !HasAuraType(SPELL_AURA_WATER_WALK));
                if (max_z > INVALID_HEIGHT)
                {
                    if (z > max_z)
                        z = max_z;
