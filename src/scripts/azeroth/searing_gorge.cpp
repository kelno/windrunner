...
bool GossipHello_npc_kalaran_windblade(Player *player, Creature *_Creature)
{
    ...
    if (player->GetQuestStatus(3441) == QUEST_STATUS_INCOMPLETE)
        player->ADD_GOSSIP_ITEM( 0, "Expliquez-moi ce qui dirige votre vengeance ?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
bool GossipSelect_npc_kalaran_windblade(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF:
            player->ADD_GOSSIP_ITEM( 0, "Continuez s'il vous plait", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->SEND_GOSSIP_MENU(1954, _Creature->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+1:
            player->ADD_GOSSIP_ITEM( 0, "Laissez-moi en discuter avec mes collègues", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
...
bool GossipHello_npc_lothos_riftwaker(Player *player, Creature *_Creature)
{
    ...
    if (player->GetQuestRewardStatus(7487) || player->GetQuestRewardStatus(7848))
        player->ADD_GOSSIP_ITEM(0, "Téléportez-moi au coeur du Magma", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
...
bool GossipHello_npc_zamael_lunthistle(Player *player, Creature *_Creature)
{
    ...
    if (player->GetQuestStatus(3377) == QUEST_STATUS_INCOMPLETE)
        player->ADD_GOSSIP_ITEM( 0, "Racontez-moi votre histoire", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF);
...
bool GossipSelect_npc_zamael_lunthistle(Player *player, Creature *_Creature, uint32 sender, uint32 action)
{
    ...
    {
        case GOSSIP_ACTION_INFO_DEF:
            player->ADD_GOSSIP_ITEM( 0, "Continuez...", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->SEND_GOSSIP_MENU(1921, _Creature->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+1:
            player->ADD_GOSSIP_ITEM( 0, "Au revoir", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
