...
struct npc_ranshallaAI : public npc_escortAI
{
    ...
    {
        return me->FindGOInGrid(GOB_FIRE, 10.0f);
    }
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        if (step) {
            ...
            case STEP_ALTAR_3:
                if (timer <= diff) {
                    if (Creature* priestess = Unit::GetCreature(*me, priestess1))
                        priestess->AI()->Talk(TALK_PRI_MANY_YEARS);
                    if (Creature* priestess = Unit::GetCreature(*me, priestess2))
            ...
            case STEP_ALTAR_6:
                if (timer <= diff) {
                    ...
                    if (GameObject* theGem = me->SummonGameObject(GOB_ELUNE_GEM, 5514.576660, -4917.354004, 852.392029, 5.631183, 0, 0, 0, 0, 120000)) 
                        gem = theGem->GetGUID();
            ...
            case STEP_ALTAR_10:
                if (timer <= diff) {
                    if (Creature* voice = Unit::GetCreature(*me, voiceOfElune))
                        voice->AI()->Talk(0);
...
bool GOHello_altar_of_elune(Player* player, GameObject* go)
{
    //if (player->GetQuestStatus(QUEST_GUARDIANS_ALTAR) == QUEST_STATUS_INCOMPLETE) {
        if (Creature* ranshalla = player->FindCreatureInGrid(10300, 30.0f, true))
...
void AddSC_winterspring()
{
    ...
    newscript->pGOHello = &GOHello_altar_of_elune;
    newscript->RegisterSelf();
}
