...
struct TRINITY_DLL_DECL instance_zulfarrak : public ScriptedInstance
{
    ...
    void SpawnPyramidWave(uint32 wave)
    {
        for (int i = 0; i < pyramidSpawnTotal; i++) {
            if (pyramidSpawns[i][0] == (float)wave) {
                ZFPosition pos = {pyramidSpawns[i][2], pyramidSpawns[i][3], 8.87, 0};
                if (Player* plr = GetPlayerInMap()) {
                    Creature* ts = plr->SummonCreature(pyramidSpawns[i][1],pos.x, pos.y, pos.z, pos.orientation, TEMPSUMMON_DEAD_DESPAWN, 600000);
